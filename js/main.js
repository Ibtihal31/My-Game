 ...
 function Mygame() {
    ...
    // Declare array for the questions and answers 
    questions = [
     ...
     'Who was the first person to sail round the world? ',
    ...
    answers = [
     ['1988', '1991', '1998', '1992'],
     ['India', 'China', 'USA', 'UK'],
     ['USA', 'UK', 'Brazil', 'Uruguay'],
     ['Robert Walpole', 'Magellan', 'Henry Waterloo', 'George senior'],
     ['USA', 'Saudi Arabia', 'China', 'Russia'],
     ['UK', 'Russia', 'India', 'USA'],
     ['Marie Curie', 'Anne Frank ', 'Rosa Parks', 'Jane Austen'],
     ['Colombia', 'India', 'Brazil', 'Ethiopia'],
     ['Two', 'Six', 'Five', 'Three'],
     ['London', 'Amsterdam', 'New York', 'Vienna']
    ],
    correctAnswers = [2,1,3,1,0,1,0,2,2,3],
    gameAnswers = [];
 
    //Setup styles and events function
    ...
    // Function to resizes the window which calculate new dimensions
     
     self.windowWasResized = function() {
     page.style.height = (h -20) +'px';
     page.style.width = (w - 20) + 'px';
    };
 
    //Setup the game page and fire off the animations
    ...
    self.startGamePlay = function() {
     ...
     startAnimation.to([startButton, title1,intro,steps], 1, {alpha:0});
     startAnimation.to([startButton, title1,intro,steps], 0.1, {css:{display:'none'}});
    ...
    // Function to rebuilds the UI with a new question and answer
    ...
    // Function called to start a gameplay timer that runs every second
    ...
    */
     ...
     * Displays a modal window with the option to exit or play again
     **/
    self.runEndOfGame = function() {
   
     window_width = window.innerWidth|| document.documentElement.clientWidth
                    || document.body.clientWidth;
     var exitButton = '<button id="exit" class="left exit" onClick="self.exit()">EXIT</button>';
     ...
     var insertedHTML = actualScoreHeader +'<div>' + exitButton + playAgainButton + '</div>';
    ...
    self.exit = function() {
     startAnimation.to([gameHeader, gameChoices], 0.1, {css:{display:'none'}});
     startAnimation.to([startButton, title1,intro,steps], 0.1, {css:{display:'block'}});
    }
  ...
  // Used to call the Mygame function
